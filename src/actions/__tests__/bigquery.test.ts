...
import { bigQuery as actionInterface } from '..'
...
describe('BigQuery Action', () => {
  ...
  const mockGcpCredentials = {
    ...
    private_key:
      '-----BEGIN PRIVATE KEY-----\nMIIEvQIBADANBgkqhkiG9w0BAQEFAASCBKcwggSjAgEAAoIBAQC9QFi67tH5\n-----END PRIVATE KEY-----\n',
  ...
  beforeEach(() => {
    clearMocks()
    jest.clearAllMocks()
    mockQuery = jest.fn().mockResolvedValue(mockQueryResult)
    const mockBigQueryInstance = {
      query: mockQuery,
    }
    jest
      .mocked(BigQuery)
      .mockImplementation(() => mockBigQueryInstance as unknown as BigQuery)
  })
